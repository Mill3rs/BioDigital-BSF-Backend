...
router.post('/complete-admin-profile', authenticate, uploadSingle('profile_image'), authController.completeAdminProfile);

// Complete location setup – final onboarding step
router.post('/complete-location', authenticate, [
  ...
  try {
    ...
    const locationJson = { country, city, address, landmark: landmark || null, lat: lat || null, lng: lng || null };

    // Persist location on the role-specific profile
    if (role === 'SUPPLIER') {
      await prisma.supplierProfile.upsert({
        where: { userId },
        create: { userId, collectionAddress: locationJson, primaryProducts: [], wasteTypes: [] },
...
router.post('/verify-company-code', authenticate, [
  ...
  try {
    ...
    if (role === 'SUPPLIER') {
      await prisma.supplierProfile.upsert({
        where: { userId },
        create: { userId, adminId: admin.id, primaryProducts: [], wasteTypes: [] },
    ...
    let roleLabel;
    if (role === 'SUPPLIER') { roleLabel = 'Supplier'; }
    else if (role === 'DRIVER') { roleLabel = 'Driver'; }
    else { roleLabel = role; }
    ...
    notificationService.notifyAdminsAndManagers(
      ...
      'SYSTEM',
      { userId: updatedUser.id, userRole: role, companyName: admin.companyName },
      'user:registered',
      { userId: updatedUser.id, fullName: updatedUser.fullName, role, companyName: admin.companyName }
    ).catch(() => {});
  } catch (error) {
    next(error);
  }
