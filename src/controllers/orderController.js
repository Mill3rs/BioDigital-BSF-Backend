...
const { calculatePagination, generateOrderNumber } = require('../utils/helpers');
  ...
  async getOrderStats(req, res, next) {
    try {
      ...
      const monthlyOrders = await prisma.$queryRaw`
        ...
        FROM orders
        ${where.customerId ? `WHERE customer_id = ${where.customerId}` : where.farmId ? `WHERE farm_id = ${where.farmId}` : ''}
